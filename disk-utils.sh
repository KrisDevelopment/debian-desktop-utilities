...
# DRAW SCRIPT MENU
echo "This is a collection of disk utilities such as helping you format/mount/unmount a disk permanently or temporarily on your system, create bootable USB, and more."
...
select option in "${options[@]}"; do
	...
	"List Devices")
		...
		script_path="$script_dir/ls-devices.sh"
		bash "$script_path" || {
			echo "Script not found: $script_path"
			exit 1
		}
	...
	"Create Bootable USB")
		# runs ./create-boot-usb-safe.sh
		script_path="$script_dir/create-boot-usb-safe.sh"
		if [ -f "$script_path" ]; then
			bash "$script_path"
		else
			echo "Script not found: $script_path"
